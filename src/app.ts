...
import "./controllers/rotational-strategy/rotational-strategy-controller";
...
(async () => {
  ...
  container
    .bind<RotationalStrategy>(TYPES.RotationalStrategy)
    .to(RotationalStrategy)
    .inSingletonScope();
  ...
  const marketBreadthService = container.get<MarketBreadthService>(
    TYPES.MarketBreadthService
  );

  console.time("Market Breadth Service Initialization");
  await marketBreadthService.initializeMarketBreadths();
  console.timeEnd("Market Breadth Service Initialization");

  const rotationalStrategy = container.get<RotationalStrategy>(
    TYPES.RotationalStrategy
  );

  console.time("RotationalStrategy Initialization");
  await rotationalStrategy.initializeSP500RotationalStrategy();
